// SCRIPT.JS DEFINITIVO - V4
// CORRECCIÓN DE URLS DUPLICADAS Y CORS
...
async function getStreamUrl() {
  try {
    ...
    // AQUÍ ESTÁ EL FIX CLAVE: Limpiar la URL que viene de la hoja
...
if (document.getElementById("videoFrame") && !document.getElementById("adminPanel")) {
  ...
  // Función de espera robusta para Video.js
  ...
  window.addEventListener("load", async () => {
    ...
    if (url.includes(".m3u8")) {
      ...
      // Destruir player anterior si existe para evitar errores
      ...
      // Manejo de errores específicos de HLS
  ...
  setInterval(async () => {
    ...
    // Comparación simple para evitar recargas innecesarias
...
async function updateStream() {
  ...
  url = sanitizeUrl(url); // Limpiar input del usuario

  if (!url) return alert("URL requerida");

  document.getElementById("videoUrlInput").value = url; // Mostrar URL limpia
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (urlIn) urlIn.onkeypress = e => e.key === "Enter" && updateStream();
});
